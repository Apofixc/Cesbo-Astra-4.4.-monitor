...
*   **Многоканальная доставка**: Поддержка HTTP (Webhooks), WebSocket (Real-time), Console (Лог Astra) и Lua Callback. Включает автоматические повторы (Retries) с экспоненциальной задержкой и пакетную отправку (Batching).
