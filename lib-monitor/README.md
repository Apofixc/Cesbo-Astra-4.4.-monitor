...
*   **Оптимизация производительности**:
    ...
    *   **JIT-фильтрация**: Автоматическая компиляция условий фильтрации в Lua-код. Поддерживает вложенные поля (например, `total.bitrate`), сложные логические группы (вложенные `and`/`or`) и оптимизированный оператор `in` через upvalues для максимальной производительности без аллокаций в рантайме.
