...
*   **Мониторинг DVB-тюнеров**: Инициализация, запуск, динамическая перенастройка (TP), получение детальных метрик (Signal, SNR, BER, UNC) и расчет интегрального показателя качества (Quality). Поддержка побитового анализа статуса адаптера (Lock, Signal, Carrier и др.) через `bit32`. Автоматическое управление счетчиком каналов Astra для совместного использования адаптеров. Прошли **Ultra-Extreme** тестирование на устойчивость к дребезгу сигнала и некорректным аппаратным метрикам.
*   **Мониторинг каналов**: Создание, обновление, поиск и удаление мониторов для потоков каналов. Поддержка различных типов мониторов: `input` (вход), `output` (выход канала) и `ip` (прямой мониторинг UDP/RTP). Прошли **Chaos Testing** на устойчивость к некорректным данным от анализатора Astra и массовому пересозданию (Churn).
*   **Объектно-ориентированный подход**: Каждый монитор является экземпляром класса (`TunerMonitor`, `ChannelMonitor`) with собственным состоянием и методами управления.
*   **Система подписок (Event Dispatcher)**: Высокопроизводительная шина событий. Поддерживает приоритеты, асинхронную обработку и кэш последних состояний (LVC) с O(1) вытеснением. Реализована гибридная модель доставки (Smart Emit), обеспечивающая мгновенную рассылку простым подписчикам в обход очереди. Подтвержденная производительность: более 1,000,000 доставок событий за 30 секунд под экстремальной нагрузкой (Stress Test V13). Прошла **Ultra-Extreme** тестирование (4 уровня сложности), включая проверку на рекурсивные вызовы и блокировки.
...
*   **Многоканальная доставка**: Поддержка HTTP (Webhooks), WebSocket (Real-time), Console (Лог Astra) и Lua Callback. Включает автоматические повторы (Retries) с экспоненциальной задержкой и пакетную отправку (Batching). WebSocket-транспорт оптимизирован для работы в условиях нестабильных соединений и массовых рассылок.
*   **Мониторинг системных ресурсов**: Детальный сбор метрик процесса Astra (CPU User/System, RAM RSS/Virtual, Threads, Uptime) и получение списка сетевых интерфейсов.
    *   **Прецизионный расчет**: Использование `os.clock()` для микросекундной точности CPU.
    *   **Оптимизация**: Парсинг `/proc` с ранним выходом для минимизации I/O.
    *   **Диагностика памяти**: Отслеживание динамики Lua GC (`lua_delta`) и фиксация базового уровня после сборки мусора (`lua_post_gc`) для выявления медленных утечек.
    *   **Сглаживание**: Поддержка Moving Average для устранения пиковых колебаний CPU. Прошел **Ultra-Extreme** тестирование на устойчивость к саботажу `/proc` и точность расчета при скачках нагрузки.
    *   **Событийная модель**: Автоматическая генерация событий `sys:resource_warning` при превышении порогов (CPU > 90%, RAM > 80%).
    *   **Сглаживание**: Поддержка Moving Average для устранения пиковых колебаний CPU.
*   **Оптимизация производительности**:
    ...
    *   **JIT-фильтрация с инлайнингом**: Автоматическая компиляция условий фильтрации в Lua-код с встраиванием (inlining) путей доступа к данным. Это исключает вызовы функций-аксессоров в горячем цикле. Поддерживает вложенные поля, сложные логические группы и оптимизированный оператор `in` через upvalues.
    *   **Decision Tree Routing**: Маски событий (Wildcards) организуются в дерево решений. Это позволяет `SubscriptionManager` находить всех подходящих подписчиков за один проход по строке типа события, независимо от количества активных масок.
    *   **Оптимизация шины событий**: Использование круговых буферов для очередей и LVC, минимизация системных вызовов времени и Zero-Closure Path при рассылке уведомлений. Встроена защита от циклических ссылок в данных событий через `visited_cache`.
    *   **Load Shedding**: Автоматическая защита от перегрузок. При заполнении очередей более чем на 90% система начинает отбрасывать низкоприоритетные события (`LOW`), а при 95% — события среднего приоритета (`MEDIUM`), сохраняя стабильность критических узлов.
    *   **Hybrid LVC (Zero-Copy Init)**: Кэширование последних состояний одновременно в виде Lua-таблиц и готовых JSON-строк. Это позволяет мгновенно инициализировать новых WebSocket/HTTP подписчиков без затрат на `json.encode`, обеспечивая экстремальную производительность при массовых подключениях.
    *   **WebSocket Batching (Opt-in)**: Поддержка пакетной отправки событий через WebSocket. Клиенты могут включить режим батчинга командой `batch`, что объединяет события в JSON-массивы и резко снижает количество системных вызовов `send` и нагрузку на сетевой стек.
    *   **Адаптивная обработка**: Автоматическое увеличение лимитов обработки событий (Batch Limit) до 1000 при росте нагрузки на очереди, что позволяет системе быстрее «разгребать» заторы.
    *   **Smart Emit (Hybrid Delivery)**: Интеллектуальный выбор пути доставки. Если подписчики не используют фильтры и троттлинг, события доставляются мгновенно через **Direct Multicast** с однократной сериализацией JSON. Это снижает задержку (latency) на 30% и полностью исключает создание объектов событий для простых уведомлений.
...
│   │   └── tuner_monitor.lua # Класс для мониторинга DVB-тюнера.
...
1.  **Классы мониторов**: Реализуют логику конкретных экземпляров (`TunerMonitor`, `ChannelMonitor`). Каждый объект инкапсулирует свое состояние, таймеры и callback-функции Astra.
...
Библиотека `lib-monitor` поставляется с развитой инфраструктурой автоматизированного тестирования, обеспечивающей 100% покрытие ключевых модулей (20 модулей, более 100 тест-кейсов).
...
*   **`LogBatchEnabled`**: Включить пакетное логирование (по умолчанию false).
*   **`LogBufferSize`**: Размер диагностического буфера логов (0 - выключено).
*   **`MaxLogQueueSize`**: Максимальный размер очереди пакетного логирования (по умолчанию 200).
*   **`MaxLogComponents`**: Лимит отслеживаемых компонентов в буфере (по умолчанию 100).
...
*   **`HttpTimeout`**: Таймаут HTTP-запросов в секундах (по умолчанию 10).
...
*   **`MaxRetries`**: Максимальное количество попыток HTTP-уведомлений (по умолчанию 5).
*   **`RetryDelay`**: Базовая задержка повтора в секундах (по умолчанию 5).

### Лимиты мониторов
*   **`ChannelMonitorLimit`**: Лимит одновременно активных мониторов каналов (по умолчанию 200).
*   **`DvbMonitorLimit`**: Лимит одновременно активных DVB-мониторов (по умолчанию 20).
*   **`MaxMonitorNameLength`**: Максимальная длина имени монитора (по умолчанию 64).
*   **`ForceSendInterval`**: Интервал принудительной отправки статуса (в циклах callback-а, по умолчанию 300).

### Системные ресурсы и GC
*   **`CpuThreshold`**: Порог использования CPU для генерации предупреждений (по умолчанию 90%).
*   **`RamThresholdPct`**: Порог использования RAM Lua для генерации предупреждений (по умолчанию 80%).
*   **`FdThreshold`**: Порог открытых файловых дескрипторов (по умолчанию 800).
*   **`MemoryLimitMb`**: Лимит памяти для адаптивного GC и планировщика (по умолчанию 50MB).
*   **`SchedulerInterval`**: Интервал тика планировщика в секундах (по умолчанию 1).
*   **`ConfigRefreshInterval`**: Интервал обновления кэша конфигурации в модулях (по умолчанию 10 сек).
*   **`AdaptiveTickThresholdCpu`**: Порог CPU для перехода на ускоренный интервал опроса (по умолчанию 50%).
*   **`AdaptiveTickThresholdRam`**: Порог RAM для перехода на ускоренный интервал опроса (по умолчанию 70%).
*   **`TickIntervalNormal`**: Обычный интервал опроса системных ресурсов (по умолчанию 5 сек).
*   **`TickIntervalFast`**: Ускоренный интервал опроса при высокой нагрузке (по умолчанию 1 сек).
*   **`RareMetricInterval`**: Интервал сбора редких метрик, таких как FD и Threads (в итерациях, по умолчанию 5).
*   **`MaxCpuJump`**: Максимально допустимый скачок CPU за один тик для фильтрации аномалий (по умолчанию 50%).
*   **`MaxRamJumpPct`**: Максимально допустимый скачок RAM за один тик (по умолчанию 20%).

### Восстановление
*   **`AutoRecoverEnabled`**: Включить автономное восстановление через планировщик (по умолчанию false).
*   **`AutoRecoverInterval`**: Интервал автоматического восстановления зависших мониторов в секундах (по умолчанию 300).
*   **`MaxRecoveryAttempts`**: Максимальное количество попыток восстановления монитора (по умолчанию 3).
*   **`RecoveryCooldown`**: Время стабильной работы в секундах для сброса счетчика попыток (по умолчанию 3600).

### События и Пакетная отправка
*   **`LvcTtl`**: Время жизни записей в Last Value Cache в секундах (по умолчанию 3600).
*   **`MaxLvcSize`**: Максимальный размер LVC (по умолчанию 1000).
*   **`MaxQueueSize`**: Максимальный размер очереди событий на один приоритет (по умолчанию 1000).
*   **`EventBatchLimit`**: Лимит обработки событий за один тик планировщика (по умолчанию 100).
*   **`BatchEnabled`**: Включить пакетную отправку событий подписчикам (по умолчанию true).
*   **`BatchFlushInterval`**: Интервал сброса буфера батчей в секундах (по умолчанию 0.5).
*   **`BatchMaxSize`**: Максимальный размер пачки событий (по умолчанию 50).

### Пулы и Кэши
*   **`MaxPoolSize`**: Максимальное количество таблиц в пуле одного типа (по умолчанию 100).
*   **`PoolDebug`**: Режим отладки пулов (валидация чистоты таблиц).
*   **`MaxCacheSize`**: Лимиты кэшей для Wildcard (1000), FilterEngine (500) и маршрутов (1000).
