...
*   **Система подписок (Event Dispatcher)**: Высокопроизводительная шина событий (~730,000 обработанных событий в секунду). Поддерживает приоритеты, асинхронную обработку и кэш последних состояний (LVC) с O(1) вытеснением. Реализована гибридная модель доставки (Smart Emit), обеспечивающая мгновенную рассылку простым подписчикам в обход очереди.
