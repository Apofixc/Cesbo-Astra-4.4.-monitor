...
*   **Оптимизация производительности**:
    ...
    *   **Пул таблиц (Table Pooling)**: Динамическое переиспользование таблиц для отчетов и событий снижает нагрузку на Garbage Collector и предотвращает фрагментацию памяти. Поддерживает глубокую очистку для сложных структур и быструю очистку через `next`. Размер пула настраивается через `MaxPoolSize`.
