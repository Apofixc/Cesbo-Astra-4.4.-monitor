...
*   **`MaxRecoveryAttempts`**: Максимальное количество попыток восстановления монитора перед его принудительной остановкой (по умолчанию 3).
*   **`RecoveryCooldown`**: Время стабильной работы монитора в секундах для сброса счетчика попыток восстановления (по умолчанию 3600).
