...
*   **GET `/api/channels`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получает список всех каналов, настроенных в системе Astra может только через обращение к таблице channels_list. Если хотите получить поле display_name, то это нужно искать этот канал в списке мониторов ChannelRepository.
```json
        [
          {
            "name": "Discovery",
...
*   **GET `/api/channels/stats`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: total_astra_channels можно получить подсчитав количество каналов в channels_list. Остальные поля можно получить только с ChannelRepository, после анализа всех мониторов    
```json
        {
...
*   **GET `/api/channels/{id}/psi`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Все эти данные можно получить из ChannelRepository. Структура PSI-таблицы будет очень сложная.
```json
        {
          "name": "Discovery",
          "display_name": "Discovery HD",          
...
*   **GET `/api/dvb/adapters/{id}/data`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из DvbRepository объект монитора и обратится к полю self._json_cache, чтобы получить готовый json с последней отправки данных.
```json
        {
          "type": "dvb",
          "server": "astra-1",
...
*   **GET `/api/monitors`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из ChannelRepository.
```json
        [
...
*   **GET `/api/monitors/{id}/data`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из ChannelRepository объект монитора и обратится к полю self._json_cache, чтобы получить готовый json с последней отправки данных.
```json
        {
          "type": "Channel",
          "server": "astra-1",
...
*   **GET `/api/monitors/{id}/pids`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из ChannelRepository объект монитора и обратится к функций get_stats().
```json
        {
...
*   **GET `/api/monitors/{id}/rate_stat`**
    ...
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из ChannelRepository объект монитора и обратится к функций get_rate_stat().


*   **POST `/api/monitors/{id}/pids/clear`**
    *   **Описание**: Сбрасывает накопленную статистику по PID и rate_stat.
    *   **Ответ**: `HTTP 200 OK`.
    *   **Способ реализаций**: Получить из ChannelRepository объект монитора и обратится к функций clear_stats().
```json
        {
