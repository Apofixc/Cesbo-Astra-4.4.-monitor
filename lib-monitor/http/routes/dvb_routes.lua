...
function DvbRoutes.get_adapter_data(server, client, request)
    ...
    if dvb_obj._json_cache then
        HttpHelpers.send_raw_json(server, client, 200, dvb_obj._json_cache)
        return true
    end
...
function DvbRoutes.update_adapter(server, client, request)
    ...
    local name = params.name
    local success, result_err = Adapter.update_dvb_monitor_parameters(name, params)
    if success then
        HttpHelpers.success(server, client, { message = "Adapter monitor updated" })
        return true
    else
        return false, result_err or "Failed to update adapter monitor"
    end
end
...
function DvbRoutes.stop_adapter(server, client, request)
    ...
    local success, result_or_err = Adapter.stop_dvb_monitor(name, force)
    if success and result_or_err then
        ...
        return false, result_or_err or "Failed to stop adapter monitor"
    end
end
...
function DvbRoutes.tune_adapter(server, client, request)
    ...
    if success and result_or_err then
        ...
        return false, result_or_err or "Failed to tune adapter"
    end
end
...
function DvbRoutes.switch_transponder(server, client, request)
    ...
    local success, result_or_err = Adapter.switch_transponder(name, data, data.reserve_input)
    if success and result_or_err then
        HttpHelpers.success(server, client, { 
        ...
        return false, result_or_err or "Failed to switch transponder"
    end
end
...
function DvbRoutes.pause_adapter(server, client, request)
    ...
    if success then
        ...
        return false, result_err or "Failed to pause adapter monitor"
    end
end
...
function DvbRoutes.resume_adapter(server, client, request)
    ...
    if success then
        ...
        return false, result_err or "Failed to resume adapter monitor"
    end
end
...
function DvbRoutes.restart_adapter(server, client, request)
    ...
    local success, result_err = Adapter.restart_dvb_monitor(name, force, params)
    if success then
        HttpHelpers.success(server, client, { message = "Adapter restarted successfully" })
        return true
    else
        return false, result_err or "Failed to restart adapter monitor"
    end
end
